...
declare -r ignite_user="ignite"  # user on remote hosts for ignite
...
for server in $(cat "${server_list}"); do 
    ...
    if [ ${ignite_runned} -eq 0 ]; then
        ...
        ignite_version=$(ssh "${ssh_conn_str}" "sudo -u ignite find /opt/ignite/server/libs/ -type f -name ignite-core*.jar 2>/dev/null | grep -Po '[0-9.]+'")
     ...
     if [ $(ssh "${ssh_conn_str}" "sudo -u ignite chage -l ignite | grep never | wc -l") -eq 3 ]; then
        ignite_chage="OK"
     else
        ignite_chage="Необходимо сменить пароль!"
     fi 
     if [ $(ssh "${ssh_conn_str}" "sudo -u ignite sudo -l | grep '\(ALL\)' | wc -l") -ge 30 ]; then
        ignite_sudo="OK"
     else
     ...
     if [ $(ssh "${ssh_conn_str}" "grep 'PIDFile=/opt/ignite/logs/ignite-server.pid' /etc/systemd/system/ignite-server.service | wc -l" ) -ne 1 ]; then
