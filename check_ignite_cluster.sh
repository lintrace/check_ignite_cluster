...
user_check () {
    ...
    local sudo_str=""
    if [ "${user}" != "${maintenance_user}" ]; then
        sudo_str="sudo -u ${user}"
    fi

    if [ $(ssh ${ssh_conn} "${sudo_str} chage -l ${user}" | grep -c "never") -ge 3 ]; then
        local user_chage="OK"
    else
        local user_chage="Необходимо сменить пароль и/или отключить срок действия пароля!"
    fi
    if [ $(ssh ${ssh_conn} "${sudo_str} sudo -l" | grep -c "${sudo_check_string}") -ge "${sudo_check_matches}" ]; then
        local user_sudo="OK"
    else
...
for server in $(cat "${server_list}"); do
    ...
    if [ -z "${ps_cmd_out}" ]; then
        ...
        # Определяем персистентность по конфигурации СУ
        run_ignite_conf_path="$( echo "${ps_cmd_out}" | grep -Po "[^' ']+.xml")"
        run_ignite_type="in-memory"
        if [ "$(ssh ${ssh_conn_str} "${sudo_str} grep -Pic 'persistenceEnabled.+true' "${run_ignite_conf_path}"  ")" -ge "1" ]; then
            run_ignite_type="PERSISTENCE - требуется активация/деактивация!"
        fi

        echo -ne "\n# Запущен Ignite версии ${run_ignite_version} под пользователем ${run_ignite_user}, тип: ${run_ignite_type}\n# Проверка портов: 22-OK"
